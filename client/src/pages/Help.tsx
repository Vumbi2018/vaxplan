...
const defaultFaqs = [
  ...
  },
  {
    ...
    answer:
      "Open Settlement Intelligence. On any zero-dose cluster card or settlement record, click the compass 'Insights' button. The Geospatial Insights panel shows the travel time to the nearest facility and the nearest existing outreach site on the real road network (each with a driving and a walking estimate, plus the road distance), and the community assets within 3 km — schools, places of worship, markets, water points, and transport nodes — pulled live from OpenStreetMap. Outreach sites are often closer to a remote cluster than a fixed facility, so the panel shows whichever is relevant — or both, clearly labelled. While the panel is open, the map also highlights the inspected point and draws the route to each destination — the real road geometry when available, or a dashed straight line when it falls back to an estimate — and marks the nearest facility and outreach site, so you can judge terrain and direction at a glance. If routing is briefly unavailable it falls back to a straight-line estimate (clearly labelled), so the panel always answers. You can also turn on the 'Travel-Time Zones' layer and switch it between Walking (1/2/3-hour zones on foot), Driving (30/60/90-minute zones by vehicle, for vehicle-based outreach and supply runs), and Cycling (30/60/90-minute zones by bicycle or motorbike, for two-wheeler outreach teams) — these are drawn around every active health facility AND every active outreach site (outreach posts are often closer to a remote cluster than a fixed facility), follow the real road and path network when routing is available, and fall back to simple rings if it isn't — and the 'Community Assets' layer (assets near the map centre) from the Map Layers Control panel. These come from open data and are a planning aid, not a survey — confirm on the ground.",
  },
  {
