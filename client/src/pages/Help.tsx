...
const defaultFaqs = [
  ...
  },
  {
    ...
    answer:
      "Open the Data Sources page from the sidebar (it's near Settings and Help) — there's also a link to it under 'External Resources' on this Help page. It lists every external source VaxPlan uses, grouped by category: map tiles (OpenStreetMap, satellite imagery), administrative boundaries (geoBoundaries, GADM, UN OCHA/HDX), population and demographics (national census, WorldPop, HMIS, community surveys), health facilities and information systems (national master facility lists, DHIS2), immunization guidance (WHO, UNICEF, Gavi), and the open-source software, fonts, and icons behind the app. Each entry shows a short description, its licence, and a link to the original source. If your country has population sources set up, they're shown at the top so you can see exactly which datasets feed your calculations. The page also carries the acknowledgements crediting these providers. This page is public — anyone can view it at /data-sources without signing in (there's also a link in the footer of the public landing page), so data providers can confirm they're properly credited.",
  },
  {
