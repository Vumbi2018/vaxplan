...
export function startStockAlertDigestScheduler(): void {
  ...
  const raw = process.env.STOCK_ALERT_DIGEST_INTERVAL_HOURS;
  const hours = raw ? parseFloat(raw) : 6;
  if (!Number.isFinite(hours) || hours <= 0) {
    console.log("[stock-alert-digest] scheduler disabled (interval <= 0)");
    return;
  }
  const intervalMs = Math.max(60_000, Math.round(hours * 60 * 60 * 1000));
  console.log(`[stock-alert-digest] scheduler enabled — checking every ${hours}h`);

  const tick = () => {
    runStockAlertDigest().catch((err) => {
      console.error("[stock-alert-digest] cycle threw:", err);
    });
  };
